...
# Using the non-local means denoising function from the opencv library h should be 60.0 if the image is binarized
blured_img = cv.fastNlMeansDenoising(src=img, h=60.0, templateWindowSize=7, searchWindowSize=21)

# Saving the blured image
saveImg = Image.fromarray(blured_img)
saveImg.save(save_path)

print(
    "\nImage " + image_path + " has successfully gone through non local image denoisng and has been stored here: " + save_path)
